...
void DrawLine(HDC hdc, int x1, int y1, int x2, int y2) {
    ...
    LineTo(hdc, x2, y2);
    //Ellipse(hdc, x1, y1, x1 + 22, y1 + 22);
}
void OnPaint(HDC hdc) {
    float xPrev = -10;
    float yPrev = Function(-10);
    float xOrigin = g_clientWidth / 2;
    float yOrigin = g_clientHeight / 2;
    for (int i = -10; i <= 10; ++i) {
        float x = i;
        float y = Function(x);
        DrawLine(hdc, xPrev+xOrigin, (-yPrev+yOrigin)
            , x+xOrigin, (-y+yOrigin));
        xPrev = x;
        yPrev = y;
    }
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
        ...
        case IDM_ABOUT:
        {
            ...
            char text[1024];
            int i = 12;
            sprintf_s(text, "hello %i", i);
            MessageBoxA(hWnd, text, "Pi", MB_OKCANCEL);
            break;
        }
		case IDM_EXIT:
			DestroyWindow(hWnd);
