...
KVector FunctionTangent(float x, float delta_ = 0.0001f) {
    KVector v1;
    KVector v2;
    v1.x = x;
    v1.y = Function(v1.x);
    ...
    KVector vTangent = v2 - v1;
    vTangent.Normalize();
    return vTangent;
}

float FunctionPrime(float x, float delta_ = 0.0001f)
{
    KVector vTangent = FunctionTangent(x, delta_);
    return vTangent.y / vTangent.x;
}
